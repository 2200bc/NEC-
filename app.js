...
function calculateDerating() {
  ...
  selected.forEach(line => {
    ...
    if (line.phase === "3" && line.neutral) conductorCount += 1;
...
function balancePanel() {
  ...
  let slotMap = new Array(slots).fill(null);
  let a = 0, b = 0, c = 0;
  ...
  sorted.forEach((line, i) => {
    ...
    if (type === "1") {
      targetPhase = a <= b ? "A" : "B";
      if (targetPhase === "A") { a += line.amps; } else { b += line.amps; }
    } else {
      if (a <= b && a <= c) { targetPhase = "A"; a += line.amps; }
      else if (b <= c) { targetPhase = "B"; b += line.amps; }
      else { targetPhase = "C"; c += line.amps; }
    }

    const slotNum = i < slots ? i : (i % slots);
    slotMap[slotNum] = `${slotNum + 1}: ${line.name} → фаза ${targetPhase}`;
  });

  let out = slotMap.filter(Boolean).join("\n");
  out += `\n\nНагрузка: A=${a}А, B=${b}А` + (type === "3" ? `, C=${c}А` : "");
